...
class Zombie:
    ...
    def run_away_from_boy(self, r = 0.5):
        tx, ty = play_mode.boy.x, play_mode.boy.y
        self.state = 'Walk'
        self.dir = math.atan2(ty - self.y, tx - self.x)
        self.speed = RUN_SPEED_PPS
        self.x -= self.speed * math.cos(self.dir) * game_framework.frame_time
    ...
    def build_behavior_tree(self):
        ...
        a6 = Action('소년에게서 도망친다', self.run_away_from_boy)


        c1 = Condition('소년이 근처에 있는가?', self.is_boy_nearby, 7)

        c2 = Condition('소년보다 공이 적은가?', self.has_fewer_ball)
        ...
        SEQ_comparison = Sequence('보유 공 개수 비교', c2, a6)
        SEL_chase_or_runaway = Selector('소년을 추적하거나 도망친다', SEQ_comparison, a4)
        SEQ_chase_boy = Sequence('소년 추적', c1, SEL_chase_or_runaway)

        SEL_chase_or_wander = Selector('소년을 추적하거나 배회한다', SEQ_chase_boy, SEQ_wander)
